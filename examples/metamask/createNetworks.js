...
(async () => {
    ...
    console.log(`Giving 1 ETH and 1000 UST to ${address} on chain 1...`);
    const [chainUser1] = chain1.userWallets;
    await (await chainUser1.sendTransaction({
        to: address,
        value: BigInt(1e18),
    })).wait();
    await chain1.giveToken(address, 'UST', 1e7);
    const [chainUser2] = chain2.userWallets;
    await (await chainUser2.sendTransaction({
        to: address,
        value: BigInt(1e18),
    })).wait();
